    ...
    public static function save_manual_ap () /*{{{*/
    {
        ...
        foreach ($data as $idx => $row)
        {
            ...
            if ($idx > 8)
            {
                ...
                {
                    DB::RollbackTrans();
                    return 'Kode Supplier [ '.$row['A'].' ] Tidak Ditemukan !';
                }

                if ($suppid == -1)
                {
                    $doctor_id = DB::GetOne("SELECT pid FROM person WHERE nrp = ?", [$row['I']]);

                    if ($doctor_id == '')
                    {
                        DB::RollbackTrans();
                        return 'NRP [ '.$row['I'].' ] Tidak Ditemukan !';
                    }
                }
                else
                    $doctor_id = NULL;
                ...
                $record['no_inv']           = $row['C'];
                $record['tgl_faktur_pajak'] = $row['G'] ? $row['G'] : NULL;
                $record['keterangan']       = $from == 'manual' ? $keterangan : 'MIGRASI MANUAL AP : '.$row['H'];
                $record['ppn']              = 0;
                $record['ppn_rp']           = 0;
