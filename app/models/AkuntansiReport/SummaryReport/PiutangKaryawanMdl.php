...
class PiutangKaryawanMdl extends DB
{
    ...
    public static function detail ($data) /*{{{*/
    {
        ...
        /* B: Showing Data From Temp Table */
        $sql = "SELECT tmp.branch_code, tmp.nama_karyawan, SUM(tmp.opbal) AS opbal
                    , SUM(tmp.ar_inv) AS ar_inv, SUM(tmp.ar_pay) AS ar_pay
                    , SUM(tmp.closbal) AS closbal, br.bid, br.branch_name
                FROM temp_summary_ar tmp
                INNER JOIN branch br ON br.branch_code = tmp.branch_code
                WHERE 1 = 1 $addsql
                GROUP BY tmp.branch_code, tmp.nama_karyawan, br.bid, br.is_primary, br.branch_name
                ORDER BY tmp.nama_karyawan";
        $rs = DB::Execute($sql);
        /* E: Showing Data From Temp Table */
    ...
    private static function _DataSummary ($data) /*{{{*/
    {
        ...
        {
            $sql = "SELECT br.branch_code, ps.nama_lengkap AS nama_karyawan
                        , SUM(CASE WHEN DATE(gl.gldate) < '$sdate' THEN gld.debet - gld.credit ELSE 0 END) AS opbal
                        , SUM(CASE WHEN DATE(gl.gldate) BETWEEN '$sdate' AND '$edate' AND gl.jtid IN (27) THEN (gld.debet - gld.credit) ELSE 0 END) AS ar_inv
                        , SUM(CASE WHEN DATE(gl.gldate) BETWEEN '$sdate' AND '$edate' AND gl.jtid IN (28) THEN (gld.credit - gld.debet) ELSE 0 END) AS ar_pay
                    ...
                    INNER JOIN person ps ON gld.other_reff_id = ps.pid
                    INNER JOIN branch br ON br.bid = gl.bid
                    WHERE gl.jtid IN (27, 28) AND gld.gltype = (CASE WHEN gl.jtid IN (28) THEN 1 ELSE 2 END)
                        AND DATE(gl.gldate) <= '$edate' AND mc.custid = -1
                    GROUP BY br.branch_code, nama_karyawan
            ...
            while (!$rs->EOF)
            {
                $record[] = array(
                    ...
                    'nama_karyawan' => $rs->fields['nama_karyawan'],
        ...
        {
            $sql = "SELECT ps.nama_lengkap AS nama_karyawan
                        , SUM(CASE WHEN DATE(gl.gldate) < '$sdate' THEN gld.debet - gld.credit ELSE 0 END) AS opbal
                        , SUM(CASE WHEN DATE(gl.gldate) BETWEEN '$sdate' AND '$edate' AND gl.jtid IN (27) THEN (gld.debet - gld.credit) ELSE 0 END) AS ar_inv
                        , SUM(CASE WHEN DATE(gl.gldate) BETWEEN '$sdate' AND '$edate' AND gl.jtid IN (28) THEN (gld.credit - gld.debet) ELSE 0 END) AS ar_pay
                        , SUM(gld.debet - gld.credit) AS closbal
                    ...
                    INNER JOIN journal_type jt ON jt.jtid = gl.jtid
                    INNER JOIN m_customer mc ON mc.custid = gl.suppid
                    LEFT JOIN manual_ar ma ON gld.reff_id = ma.maid
                    LEFT JOIN person ps ON ma.pegawai_id = ps.pid
                    WHERE gl.jtid IN (27, 28) AND gld.gltype = (CASE WHEN gl.jtid IN (28) THEN 1 ELSE 2 END)
                        AND DATE(gl.gldate) <= '$edate' AND mc.custid = -1
