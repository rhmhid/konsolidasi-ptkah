...
<table width="100%" class="pad">
    <tbody>
        <tr>
            ...
            <td width="20%">: {{ $data_db->nama_customer }} {!! $detail_cust !!}</td>
    ...
    <tbody>
        @foreach ($rsd as $row)
            ...
            <tr>
                ...
                <td>{{ $row->arcode }}</td>
                <td>{{ $row->no_inv }}</td>
                <td align="right">Rp. {{ format_uang($row->nominal_piutang, 2) }}</td>
            </tr>
        @endforeach
    </tbody>
    <tfoot>
...
<table width="100%" class="bdr1 pad sml">
    ...
    <tbody>
        ...
        @foreach ($rsdaddless as $row2)
            @php
                $row2 = FieldsToObject($row2);

                ++$no2;
                $debet = $row2->debet;
                $credit = $row2->credit;
                $total = floatval($total) - floatval($debet) + floatval($credit);
            ...
            <tr>
                ...
                <td>{{ $row2->coa }}</td>
                <td>{{ $row2->ket_addless }}</td>
                <td align="right">Rp. {{ format_uang($row2->debet, 2) }}</td>
                <td align="right">Rp. {{ format_uang($row2->credit, 2) }}</td>
            </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr>
            <td colspan="3" align="right"><b>SUBTOTAL</b></td>
            <td align="right"><b>Rp. {{ format_uang($stdeb, 2) }}</b></td>
            <td align="right"><b>Rp. {{ format_uang($stcre, 2) }}</b></td>
        </tr>
