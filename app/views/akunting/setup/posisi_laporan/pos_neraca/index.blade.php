    ...
    <div class="flex-lg-row-fluid">
        ...
        <div class="card border border-gray-300 rounded-1">
            ...
            <div class="card-body p-6 mt-n2 border-top-3 border-dark" id="kt_content_body">
                <table id="myTablePosNa" class="table table-striped table-row-bordered border border-bottom border-gray-300 align-middle rounded-0 g-4 fs-8 w-100">
...
<!--begin::Modal - Ubah-->
<div class="modal fade" id="popup-form-pos-na" data-bs-focus="false" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="popup-form-pos-na" aria-hidden="true">
    <!--begin::Modal dialog-->
...
<script type="text/javascript">
    // DATATABLE myTablePosNa
    const ajaxOptions = {
        url     : "{{ route('api.akunting.setup.posisi_laporan.pos_neraca') }}",
    ...
    const options = {
        columns:[
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta)
                        {
                            $btnFungsi = `<a href="javascript:void(0)" data-pnid="${row.pnid}" data-jenis_pos="${row.jenis_pos}" title="Ubah" class="btn btn-dark btn-sm fs-8 btn-update-pos-na">
    ...
    $('#myTablePosNa').on('click', '.btn-update-pos-na', function ()
    {
        const pnid = $(this).data('pnid')
        const jenis_pos = $(this).data('jenis_pos')

        openPosNaModal(pnid, jenis_pos)
    })

    function openPosNaModal (pnid = 0, jenis_pos)
    {
        showLoading()

        getForm(pnid, jenis_pos).then((res) => {
            Swal.close()

            // Show modal
            openModal('popup-form-pos-na', res, null, true)
        })
    }

    async function getForm (pnid, jenis_pos)
    {
        let result
        let link = pnid == 0 ? "{{ route('api.akunting.setup.posisi_laporan.pos_neraca.create') }}" : "{{ route('api.akunting.setup.posisi_laporan.pos_neraca.edit') }}"

        try {
            result = await $.ajax({
                url     : link,
                type    : 'GET',
                data    : { pnid: pnid, jenis_pos: jenis_pos }
            })
