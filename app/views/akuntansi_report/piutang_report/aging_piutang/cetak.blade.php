    ...
    <tbody>
        ...
        @forelse ($rs as $row)
            @php
                ...
                $row->nama_customer .= $row->custid == -2 ? ' <i style="color: red">[ '.$row->bank_nama.' ]</i>' : '';

                if ($nama_customer !== null && $nama_customer !== $row->nama_customer) {
            @endphp
                <tr>
                    <td colspan="6" align="right"><b>SUBTOTAL {!! $nama_customer !!}</b></td>
            ...
            <tr>
                ...
                <td align="center">{{ dbtstamp2stringlong_ina($row->ardate) }}</td>
