...
class Modules
{
    ...
    public static function data_branch () /*{{{*/
    {
        // if (Auth::user()->pid == SUPER_USER) self::$db->debug = true;

        $bid = Auth::user()->branch->bid;

        $sql = "SELECT branch_name, bid, coaid_branch FROM branch WHERE is_aktif = 't' AND bid != ? ORDER BY branch_name";
    ...
    public static function data_supplier () /*{{{*/
    {
        ...
        $sql = "SELECT a.nama_supp, a.suppid, a.kode_supp
    			FROM m_supplier a 
    ...
    public static function data_doctor () /*{{{*/
    {
        ...
        $sql = "SELECT a.nama_lengkap, a.pid, a.nrp
                FROM person a
                INNER JOIN branch_assign b ON b.base_id = a.pid AND b.item_type = 1
                WHERE a.is_aktif = 't' AND a.is_dokter = 't' AND b.bid = ?
                ORDER BY LOWER(a.nama_lengkap)";
        $rs = self::$db->Execute($sql, [$bid]);
