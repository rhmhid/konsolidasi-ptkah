...
class DistribusiBarang extends BaseController
{
    ...
    public function list () /*{{{*/
    {
        ...
        $cmb_konfirmasi = $rs_konfirmasi->GetMenu2('', '', true, false, 0, 'class="form-select form-select-sm rounded-1 w-100" id="sKonfirm" data-control="select2" data-allow-clear="true" data-placeholder="Pilih..." ');

        return view('inventori.distribusi_barang.list', compact(
            'cmb_gudang_from',
            'cmb_gudang_to',
            'cmb_kel_brg',
            'cmb_konfirmasi'
    ...
    {
        $rsd = DistribusiBarangMdl::detail_data($myid);

        $data_db = !$rsd->EOF ? FieldsToObject($rsd->fields) : New stdClass();
        $now = dbtstamp2stringina(date('Y-m-d'));

        return view('inventori.distribusi_barang.cetak', compact(
            'myid',
            'rsd',
            'data_db',
            'now'
