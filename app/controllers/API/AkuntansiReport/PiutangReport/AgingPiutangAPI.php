...
class AgingPiutangAPI extends BaseAPIController
{
    ...
    public function excel_get () /*{{{*/
    {
        ...
        header('Content-Disposition: attachment; filename="Laporan Aging Piutang.xlsx"');
    ...
    public function excel_detail_get () /*{{{*/
    {
        ...
        $sheet->setCellValue('A1', 'LAPORAN AGING PIUTANG DETAIL');
        ...
        while (!$rs->EOF)
        {
            ...
            $is_new_customer = ($current_customer !== null && $current_customer !== $row->nama_customer);

            if ($is_new_customer) {
                $sheet->setCellValue('A'.$row_pos, 'SUBTOTAL ' . strtoupper($current_customer));
            ...
            $current_customer = $row->nama_customer;
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Laporan Aging Piutang Detail.xlsx"');
        header('Cache-Control: max-age=0');
