...
class AgingKaryawanAPI extends BaseAPIController
{
    ...
    public function __construct () /*{{{*/
    {
        ...
        $this->load->model('AkuntansiReport/PiutangReport/AgingKaryawanMdl');
    ...
    public function excel_get () /*{{{*/
    {
        ...
        $sheet->setCellValue('A1', 'LAPORAN AGING KARYAWAN');
        ...
        $sheet->setTitle("Aging Piutang Karyawan");

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Laporan Aging Piutang Karyawan.xlsx"');
    ...
    public function excel_detail_get () /*{{{*/
    {
        ...
        $rs = AgingKaryawanMdl::detail($data);
        ...
        $sheet->setCellValue('A1', 'LAPORAN AGING PIUTANG KARYAWAN DETAIL');
        ...
        while (!$rs->EOF)
        {
            ...
            $is_new_karyawan = ($current_karyawan !== null && $current_karyawan !== $row->nama_karyawan);

            if ($is_new_karyawan) {
                $sheet->setCellValue('A'.$row_pos, 'SUBTOTAL ' . strtoupper($current_karyawan));
            ...
            $current_karyawan = $row->nama_karyawan;
            ...
            $sheet->setCellValue('A'.$row_pos, $no++);
            $sheet->setCellValue('B'.$row_pos, $row->nama_karyawan);
        ...
        $sheet->getDefaultRowDimension()->setRowHeight(-1);
        $sheet->getPageSetup()->setOrientation(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_LANDSCAPE);
        $sheet->setTitle("Detail Aging Piutang Karyawan");

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Laporan Aging Piutang Karyawan Detail.xlsx"');
        header('Cache-Control: max-age=0');
