...
class HutangDokterAPI extends BaseAPIController
{
    ...
    public function excel_get () /*{{{*/
    {
		...
		$sheet->setCellValue('A1', 'SUMMARY REPORT A/P DOCTOR');
        ...
        $sheet->setTitle("Summary Report AP Doctor");

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Summary Report AP Doctor.xlsx"');
    ...
    public function excel_detail_get () /*{{{*/
    {
		...
		$sheet->setCellValue('A1', 'SUMMARY REPORT A/P DOCTOR DETAIL');
        $sheet->mergeCells('A1:F1');
        $sheet->getStyle('A1')->getFont()->setSize(16)->setBold(true);
        ...
        $sheet->setCellValue('A5', 'No.');
        $sheet->setCellValue('B5', 'Nama Dokter');
        $sheet->setCellValue('C5', 'Begining Balance Total');
        $sheet->setCellValue('D5', 'A/P Doctor Invoice');
        $sheet->setCellValue('E5', 'A/P Doctor Payment');
        $sheet->setCellValue('F5', 'Ending Balance');
        ...
        while (!$rs->EOF)
        {
            ...
            $sheet->setCellValue('A'.$row_pos, $no++);
            $sheet->setCellValue('B'.$row_pos, $row->nama_dokter);
            $sheet->setCellValue('C'.$row_pos, $opbal);
            $sheet->setCellValue('D'.$row_pos, $ap_inv);
            $sheet->setCellValue('E'.$row_pos, $ap_pay);
            $sheet->setCellValue('F'.$row_pos, $closbal);
