...
class PosplAPI extends BaseAPIController
{
    ...
    public function list_data_get () /*{{{*/
    {
        ...
        while (!$rs->EOF)
        {
            ...
            $record[] = array(
                ...
                "pos_rl_rekap"  => $rs->fields['pos_rl_rekap'],
    ...
    public function form_get () /*{{{*/
    {
        ...
        $rs_pos_rekap = PosplMdl::pos_rekap();
        $cmb_pos_rekap = $rs_pos_rekap->GetMenu2('pplrid', $data_pos->pplrid, true, false, 0, 'class="form-select form-select-sm rounded-1 w-100" id="pplrid" data-control="select2" data-allow-clear="true" data-placeholder="Pilih..."');
    ...
    public function list_data_rekap_get () /*{{{*/
    {
        $data = array(
            'draw'      => get_var('draw'),
            'start'     => get_var('start'),
        ...
        $jmlbris = PosplMdl::list_rekap($data, true)->RecordCount();
        $rs = PosplMdl::list_rekap($data, false, $data['start'], $data['length']);
    ...
    public function form_rekap_get () /*{{{*/
    {
        $pplrid = get_var('pplrid', 0);

        $rsd = PosplMdl::posisi_detail_rekap($pplrid);
        ...
        $rs_parent_post = PosplMdl::parent_post_rekap($data_pos->pplrid);
        $cmb_parent_pos = $rs_parent_post->GetMenu2('parent_pplrid', $data_pos->parent_pplrid, true, false, 0, 'class="form-select form-select-sm rounded-1 w-100" id="parent_pplrid" data-control="select2" data-allow-clear="true" data-placeholder="Pilih..."');

        $chk_aktif = $is_aktif == 't' ? 'checked=""' : '';
        $txt_aktif = get_status_aktif($is_aktif);

        return view('akunting.setup.posisi_laporan.pos_pl.form_rekap', compact(
            'data_pos',
            'cmb_jenis_pos',
            'cmb_parent_pos',
            'chk_aktif',
    ...
    public function save_rekap_patch () /*{{{*/
    {
        $msg = PosplMdl::save_rekap_posisi();
