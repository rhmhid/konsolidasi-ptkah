...
class MigrasiDataAPI extends BaseAPIController
{
    ...
    public function download_file_manual_ap_get () /*{{{*/
    {
        ...
        $sheet->setCellValue('I9', "nrp_dokter");

        $sheet->setCellValue('A10', "KODE SUPPLIER/SUB Dokter *");
        $sheet->setCellValue('B10', "NAMA SUPPLIER/SUB Dokter *");
        ...
        $sheet->setCellValue('I10', "NRP DOKTER");
        ...
        $sheet->getStyle('A10:I10')->applyFromArray($style_col);
        ...
        $sheetsupplier->getStyle('A1:C1')->applyFromArray($style_colA); // ameh otomatis set width na

        $data_supp = Modules::data_supplier();
        ...
        // B: Sheet Doctor
        $sheetdoctor = $spreadsheet->createSheet();
        $sheetdoctor = $spreadsheet->getActiveSheet();
        $sheetdoctor = $spreadsheet->setActiveSheetIndex(2);

        $sheetdoctor->setTitle("Master Data Dokter");
        $sheetdoctor->setCellValue('A1', "NO");
        $sheetdoctor->setCellValue('B1', "NRP");
        $sheetdoctor->setCellValue('C1', "Nama Dokter");

        $sheetdoctor->getStyle('A1:C1')->applyFromArray($style_colA);// ameh otomatis set width na
        ...
        {
            $row = FieldsToObject($data_doctor->fields);

            $sheetdoctor->setCellValue('A'.$mulai, $nourut);
            $sheetdoctor->setCellValue('B'.$mulai, $row->nrp);
            $sheetdoctor->setCellValue('C'.$mulai, $row->nama_lengkap);

            $sheetdoctor->getStyle('A'.$nourut.':C'.$nourut)->applyFromArray($style_colDoctor);
            $mulai++;
            $nourut++;
        ...
        foreach ($sheetdoctor->getColumnIterator() as $column)
            $sheetdoctor->getColumnDimension($column->getColumnIndex())->setAutoSize(true);

        // Set height semua kolom menjadi auto (mengikuti height isi dari kolommnya, jadi otomatis)
        $sheetdoctor->getDefaultRowDimension()->setRowHeight(-1);
        // E: Sheet Doctor
