...
class PemakaianBarangAPI extends BaseAPIController
{
    ...
    public function create_get () /*{{{*/
    {
        ...
        $cmb_coa_ciu = $rs_gudang->GetMenu2('coaid_cogs', '', true, false, 0, 'class="form-select form-select-sm rounded-1 w-100" id="coaid-cogs" data-control="select2" data-allow-clear="true" data-placeholder="Pilih..."');

        $rs_branch = Modules::data_branch();

        $cmb_branch = "<select name='to_bid' id='to-bid' class='form-select form-select-sm rounded-1 w-100' data-control='select2' data-allow-clear='true' data-placeholder='Pilih...'>\n
                        <option value='' data-coaid=''></option>\n";

        while (!$rs_branch->EOF)
        {
            $row = FieldsToObject($rs_branch->fields);

            $cmb_branch .= "<option value='".$row->bid."' data-coaid='".$row->coaid_branch."'>".$row->branch_name."</option>\n";
